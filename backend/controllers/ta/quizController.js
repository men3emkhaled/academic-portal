    ...
    try {
        const result = await db.query(
            `SELECT q.*, c.name as course_name 
             ...
             WHERE EXISTS (SELECT 1 FROM ta_courses WHERE ta_id = $1 AND course_id = q.course_id)
...
const createQuiz = async (req, res) => {
    try {
        ...
        const hasAccess = await TeachingAssistant.hasCourseAccess(req.ta.id, course_id);
        if (!hasAccess) {
    ...
    try {
        ...
        const quizCheck = await db.query(
            `SELECT q.id FROM quizzes q
             WHERE q.id = $1 AND EXISTS (SELECT 1 FROM ta_courses WHERE ta_id = $2 AND course_id = q.course_id)`,
            [id, req.ta.id]
        );
    ...
    try {
        const result = await db.query(`
            ...
            WHERE qa.status = 'pending_review' 
              AND EXISTS (SELECT 1 FROM ta_courses WHERE ta_id = $1 AND course_id = q.course_id)
    ...
    try {
        ...
        const check = await db.query(
            `SELECT qa.id FROM quiz_attempts qa
             JOIN quizzes q ON qa.quiz_id = q.id
             WHERE qa.id = $1 AND EXISTS (SELECT 1 FROM ta_courses WHERE ta_id = $2 AND course_id = q.course_id)`,
            [attemptId, req.ta.id]
        );
    ...
    try {
        ...
        const check = await db.query(
            `SELECT sa.attempt_id FROM student_answers sa
             ...
             JOIN quizzes q ON qa.quiz_id = q.id
             WHERE sa.id = $1 AND EXISTS (SELECT 1 FROM ta_courses WHERE ta_id = $2 AND course_id = q.course_id)`,
            [answerId, req.ta.id]
        );
        if (check.rows.length === 0) {
            return res.status(403).json({ message: 'Access denied' });

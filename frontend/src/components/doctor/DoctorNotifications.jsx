    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-doctor-card border border-doctor-text/10 p-6 rounded-[2.5rem] shadow-sm dark:shadow-none">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-doctor-text tracking-tight">Notifications</h1>
      ...
      <div className="bg-doctor-card border border-doctor-text/10 rounded-[2.5rem] overflow-hidden shadow-sm dark:shadow-none">
        {loading ? (
          ...
          <div className="py-20 text-center">
            <div className="w-20 h-20 bg-doctor-text/5 rounded-full flex items-center justify-center mx-auto mb-6">
              <Inbox className="w-10 h-10 text-doctor-text-muted" />
            </div>
            <h3 className="text-xl font-bold text-doctor-text mb-2">No notifications yet</h3>
          ...
          <div className="divide-y divide-doctor-text/5">
            {notifications.map((n) => (
              <div 
                key={n.id}
                onClick={() => !n.is_read && onMarkRead(n.id)}
                className={`p-6 flex gap-5 hover:bg-doctor-text/[0.02] transition-all cursor-pointer group relative ${!n.is_read ? 'bg-doctor-primary/5' : ''}`}
              >
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110 ${
                  !n.is_read ? 'bg-doctor-primary/20 text-doctor-primary' : 'bg-doctor-text/5 text-doctor-text-muted'
                }`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-2">
                    <h4 className={`text-base font-bold truncate pr-4 ${!n.is_read ? 'text-doctor-text' : 'text-doctor-text-muted'}`}>
                      {n.title}
                    </h4>

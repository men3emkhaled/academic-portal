...
const TACourseProgress = ({ courses }) => {
  ...
  const fetchProgress = async () => {
    setLoading(true);
    try {
      const res = await taApi('get', `/ta/course-progress/${selectedCourseId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await taApi('put', `/ta/course-progress/${editingItem.id}`, formData);
        toast.success(t('doctor.progress.item_updated'));
      } else {
        await taApi('post', '/ta/course-progress', {
          ...formData,
  ...
  const handleToggle = async (id, currentStatus) => {
    try {
      await taApi('patch', `/ta/course-progress/${id}/toggle`, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await taApi('delete', `/ta/course-progress/${id}`);
...
export default TACourseProgress;

...
import TAQuestionManager from './TAQuestionManager';
...
const TAQuizManager = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingQuiz) {
                await taApi('put', `/ta/quizzes/${editingQuiz.id}`, formData);
                toast.success(t('doctor.quizzes.updated_success'));
            } else {
                await taApi('post', '/ta/quizzes', formData);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await taApi('delete', `/ta/quizzes/${id}`);
    ...
    return (
        <div className="max-w-[1600px] mx-auto pb-20 px-4">
            {!selectedQuiz ? (
                ...
                <TAQuestionManager quiz={selectedQuiz} onBack={() => setSelectedQuiz(null)} />
            )}

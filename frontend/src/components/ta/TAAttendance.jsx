...
const TAAttendance = ({ courses }) => {
  ...
  const fetchSessions = async () => {
    setLoading(true);
    try {
      const res = await taApi('get', `/ta/attendance/${selectedCourseId}/sessions`);
  ...
  const fetchCourseStudents = async () => {
    try {
      const res = await taApi('get', `/ta/progress/${selectedCourseId}`);
  ...
  const fetchRecords = async () => {
    try {
      const res = await taApi('get', `/ta/attendance/sessions/${activeSession.id}/records`);
      setRecords(res.data);
    } catch (err) {
  ...
  const handleCreateSession = async () => {
    try {
      const res = await taApi('post', '/ta/attendance/sessions', { courseId: selectedCourseId });
  ...
  const handleScan = async (detectedCodes) => {
    ...
    try {
      const res = await taApi('post', '/ta/attendance/scan', {
  ...
  const handleManualToggle = async (studentId) => {
    try {
      const res = await taApi('post', '/ta/attendance/manual', {
        sessionId: activeSession.id,
        studentId
      });
  ...
  const handleUpdateSession = async () => {
    ...
    try {
      const res = await taApi('put', `/ta/attendance/sessions/${activeSession.id}`, { title: editSessionTitle });
  ...
  const handleDeleteSession = async () => {
    ...
    try {
      await taApi('delete', `/ta/attendance/sessions/${activeSession.id}`);
  ...
  const handleExportAttendance = async () => {
    try {
      const res = await taApi('get', `/ta/attendance/${selectedCourseId}/export`);

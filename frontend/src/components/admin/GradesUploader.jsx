    ...
    <div className="animate-fadeIn">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-black text-white tracking-tight flex items-center gap-3">
              <BarChart3 className="w-6 h-6 text-orange-400" /> Grade Ingestion
            </h2>
            <p className="text-slate-500 text-xs font-bold uppercase tracking-[0.2em] mt-1">Bulk Assessment Architecture</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="admin-card relative overflow-hidden group">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-10">
                        ...
                        <h3 className="text-xl font-black text-white tracking-tight">Node Configuration</h3>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5 flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Matrix Schema</p>
                                    <p className="text-xs font-bold text-slate-600 tracking-tight">ID, NAME, SCORE</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <form onSubmit={handleUploadGrades} className="space-y-6 border-t border-white/5 pt-8">
                        <div className="flex flex-col sm:flex-row gap-6 items-center">
                            <label className={`relative flex-1 flex items-center justify-center gap-3 cursor-pointer bg-slate-900 border border-white/5 border-dashed rounded-3xl p-6 hover:border-orange-500/50 hover:bg-orange-500/5 transition-all group ${uploadingGrades ? 'opacity-50 pointer-events-none' : ''}`}>
                                <FileSpreadsheet className="w-6 h-6 text-slate-500 group-hover:text-orange-400 transition-colors" />
                                <span className="text-slate-300 font-black uppercase text-xs tracking-widest truncate max-w-[250px]">
                            ...
                            <div className="flex flex-col gap-3 w-full sm:w-auto">
                                ...
                                <button
                                    ...
                                    className="px-12 h-[50px] font-black uppercase tracking-widest text-red-400 bg-red-500/10 border border-red-500/20 rounded-2xl hover:bg-red-500/20 hover:text-red-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    Clear Grades
                                </button>
                            </div>
        ...
        <div className="lg:col-span-1">
            <div className="bg-[#111111]/40 border border-white/5 rounded-[2.5rem] p-10 h-full flex flex-col justify-center items-center relative overflow-hidden text-center group">
                 ...
                 {!uploadingGrades && step === 0 ? (
                     <div className="animate-fadeIn">
                        <BarChart3 className="w-20 h-20 text-slate-800 mb-8 mx-auto" />
                        <h4 className="text-xs font-black text-slate-500 uppercase tracking-[0.4em] mb-4">Ingestion Engine</h4>
                        <p className="text-xs font-bold text-slate-600 leading-relaxed">System ready for assessment matrix synchronization. Please configure node parameters.</p>
                     </div>
                 ) : (
                     <div className="w-full space-y-10 animate-fadeIn">
                        <div className="relative">
                            <div className="w-32 h-32 rounded-full border-4 border-white/5 border-t-orange-500 animate-spin mx-auto"></div>
                            <div className="absolute inset-0 flex items-center justify-center">
                                {step === 4 ? <CheckCircle className="w-12 h-12 text-emerald-400" /> : <Activity className="w-10 h-10 text-orange-400 animate-pulse" />}
                            </div>
                        </div>

                        <div>
                            <h4 className={`text-sm font-black uppercase tracking-[0.2em] mb-4 transition-colors ${step === 4 ? 'text-emerald-400' : 'text-orange-400'}`}>
                                {getStepText()}
                            </h4>
                        ...
                        {step === 4 && (
                            <p className="text-[10px] font-black text-emerald-500/60 uppercase tracking-widest animate-bounce">Ledger Overwrite Successful</p>
                        )}
                     </div>
                 )}
            </div>

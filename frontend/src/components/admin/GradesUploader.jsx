      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 sm:gap-10 relative z-10">
        <div className="space-y-2 sm:space-y-4 max-w-2xl text-start">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-400 dark:text-white/30">{t('admin.sidebar.tabs.grades')}</span>
          </div>
        ...
        <div className="bg-gradient-to-br from-[#8b5cf6] to-[#6d28d9] text-white p-6 sm:p-8 rounded-[2.5rem] shadow-lg shadow-purple-600/20 flex flex-col justify-between relative overflow-hidden group min-w-[280px] w-full lg:w-auto">
          ...
          <div className="flex justify-between items-start relative z-10 text-start">
          ...
          <div className="mt-4 relative z-10 text-start">
            <p className="text-4xl sm:text-5xl font-black uppercase tracking-tighter">SYNC</p>
            <p className="text-[10px] font-black uppercase tracking-widest opacity-60 mt-1">{t('admin.grades.status.access')}</p>
          </div>
        </div>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 sm:gap-10 relative z-10">
        
        {/* Left Column: Settings & File Upload */}
        <div className="xl:col-span-2 space-y-8">
            <div className="bg-white/50 dark:bg-[#0d0d14] border border-gray-100 dark:border-white/5 rounded-[3rem] p-8 lg:p-12 shadow-sm relative overflow-hidden group">
                <div className="absolute -inset-inline-start-20 -top-20 w-80 h-80 bg-purple-500/5 rounded-full hidden pointer-events-none group-hover:scale-110 transition-transform duration-1000"></div>
                ...
                <div className="relative z-10 space-y-10">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    <select
                                        ...
                                        className="w-full bg-gray-50 dark:bg-black/20 text-gray-900 dark:text-white border border-gray-100 dark:border-white/10 rounded-2xl px-6 py-4.5 font-black focus:ring-4 focus:ring-[#8b5cf6]/10 outline-none transition-all shadow-inner appearance-none uppercase tracking-widest text-[11px]"
                                        disabled={uploadingGrades}
                                    >
                                        <option value="" className="bg-white dark:bg-[#0d0d14] dark:text-white">{t('admin.grades.settings.dept_placeholder')}</option>
                                        {departments.map((d) => (
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    <select
                                        ...
                                        className={`w-full bg-gray-50 dark:bg-black/20 text-gray-900 dark:text-white border border-gray-100 dark:border-white/10 rounded-2xl px-6 py-4.5 font-black focus:ring-4 focus:ring-[#8b5cf6]/10 outline-none transition-all shadow-inner appearance-none uppercase tracking-widest text-[11px] ${!selectedDepartmentId ? 'opacity-50 cursor-not-allowed' : ''}`}
                                        disabled={uploadingGrades || !selectedDepartmentId}
                                    >
                                        <option value="" className="bg-white dark:bg-[#0d0d14] dark:text-white">{t('admin.grades.settings.course_placeholder')}</option>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    <select
                                        ...
                                        disabled={uploadingGrades}
                                    >
                                        <option value="midterm" className="bg-white dark:bg-[#0d0d14] dark:text-white">{t('admin.grades.exam_types.midterm')}</option>
                                        <option value="practical" className="bg-white dark:bg-[#0d0d14] dark:text-white">{t('admin.grades.exam_types.practical')}</option>
                                        <option value="oral" className="bg-white dark:bg-[#0d0d14] dark:text-white">{t('admin.grades.exam_types.oral')}</option>
                                    </select>
                                    <ChevronRight className={`absolute inset-inline-end-6 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400 rotate-90 ${i18n.language === 'ar' ? '-rotate-90' : ''}`} />
                                </div>
                            </div>

                            <div className="bg-[#8b5cf6]/5 dark:bg-[#8b5cf6]/10 border border-[#8b5cf6]/20 rounded-2xl p-6 flex items-start gap-4 shadow-inner group-hover:bg-[#8b5cf6]/10 transition-colors">
                                <div className="w-10 h-10 rounded-xl bg-purple-500/20 flex items-center justify-center text-[#8b5cf6] shrink-0">
                                    <Info className="w-5 h-5" />
                                </div>
                                <div>
                                    <p className="text-[10px] font-black text-[#8b5cf6] uppercase tracking-[0.2em] mb-2">Schema Protocol</p>
                    ...
                    <form onSubmit={handleUploadGrades} className="space-y-8 border-t border-gray-100 dark:border-white/5 pt-10">
                        <div className="flex flex-col lg:flex-row gap-8 items-stretch">
                            {/* File Upload Area */}
                            <label className={`relative flex-1 flex flex-col items-center justify-center gap-5 cursor-pointer bg-white/30 dark:bg-white/[0.01] border-2 border-gray-100 dark:border-white/5 border-dashed rounded-[2.5rem] p-10 hover:border-[#8b5cf6]/40 hover:bg-[#8b5cf6]/5 transition-all group/label shadow-inner overflow-hidden text-center ${uploadingGrades ? 'opacity-50 pointer-events-none' : ''}`}>
                                <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 to-transparent opacity-0 group-hover/label:opacity-100 transition-opacity"></div>
                                <div className="w-16 h-16 bg-gray-50 dark:bg-white/5 rounded-full flex items-center justify-center group-hover/label:bg-purple-500/10 group-hover/label:scale-110 transition-all duration-500 shadow-inner relative z-10">
                                    <UploadCloud className="w-8 h-8 text-gray-300 dark:text-slate-700 group-hover/label:text-[#8b5cf6] transition-colors" />
                                </div>
                                
                                <span className="text-gray-500 dark:text-slate-400 font-black text-[10px] uppercase tracking-[0.2em] relative z-10">
                                    {gradesFile ? (
                                        <span className="flex flex-col items-center gap-1">
                                            <span className="text-[#8b5cf6] font-black truncate max-w-[250px] block">{gradesFile.name}</span>
                            ...
                            <div className="flex flex-col gap-5 w-full lg:w-[320px] shrink-0 justify-center">
                                <button
                                    ...
                                    className="w-full bg-[#8b5cf6] hover:bg-[#7c3aed] text-white font-black py-5 rounded-[2.5rem] shadow-xl shadow-purple-500/20 transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 group h-[70px]"
                                >
        ...
        <div className="xl:col-span-1">
            <div className="bg-white/50 dark:bg-[#0d0d14] border border-gray-100 dark:border-white/5 rounded-[3rem] p-10 lg:p-12 h-full flex flex-col justify-center items-center relative overflow-hidden text-center shadow-sm min-h-[400px]">
                 {/* Cinematic Glow */}
                 <div className="absolute top-0 inset-inline-start-0 w-full h-1.5 bg-gradient-to-r from-transparent via-[#8b5cf6] to-transparent opacity-50"></div>
                 
                 {!uploadingGrades && step === 0 ? (
                      <div className="space-y-8 animate-in fade-in zoom-in duration-700">
                        <div className="w-24 h-24 bg-gray-50 dark:bg-white/5 rounded-[2rem] flex items-center justify-center mx-auto shadow-inner border border-gray-100 dark:border-white/5 group hover:border-[#8b5cf6]/30 transition-all duration-700 hover:rotate-12">
                            <FileText className="w-10 h-10 text-gray-200 dark:text-slate-800 group-hover:text-[#8b5cf6] transition-colors" />
                        </div>
                        <div>
                      ...
                      <div className="w-full space-y-10 animate-in fade-in zoom-in duration-500">
                        <div className="relative flex justify-center">
                            {/* Spinning Ring */}
                            <div className="w-32 h-32 rounded-full border-[3px] border-gray-100 dark:border-white/5 border-t-[#8b5cf6] animate-spin shadow-2xl"></div>
                            <div className="absolute inset-0 flex items-center justify-center">
                                {step === 4 ? (
                                    <CheckCircle className="w-12 h-12 text-[#2cfc7d] animate-in zoom-in duration-500" />
                                ) : (
                        ...
                        <div className="space-y-6">
                            <h4 className={`text-xs font-black uppercase tracking-[0.3em] transition-colors duration-500 ${step === 4 ? 'text-[#2cfc7d]' : 'text-[#8b5cf6]'}`}>
                                {getStepText()}
                            </h4>
                            ...
                            <div className="w-full bg-gray-100 dark:bg-black/40 rounded-full h-3 overflow-hidden border border-gray-100 dark:border-white/10 shadow-inner p-0.5">
                                <div 
                                    className={`h-full rounded-full transition-all duration-700 ${step === 4 ? 'bg-[#2cfc7d] shadow-[0_0_15px_rgba(46,204,113,0.4)]' : 'bg-[#8b5cf6] shadow-[0_0_20px_rgba(139,92,246,0.4)]'}`} 
                        ...
                        {step === 4 && (
                            <p className="text-[10px] font-black text-[#2cfc7d] uppercase tracking-[0.4em] animate-bounce italic">

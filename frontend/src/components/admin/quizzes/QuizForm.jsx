      ...
      <div className="bg-white dark:bg-[#0d0d14] border border-gray-100 dark:border-white/5 rounded-[3.5rem] p-10 lg:p-12 w-full max-w-4xl shadow-2xl relative overflow-hidden animate-in zoom-in duration-500 max-h-[95vh] overflow-y-auto no-scrollbar" onClick={e => e.stopPropagation()}>
        ...
        <div className="relative z-10 text-start">
            ...
            <form onSubmit={handleSaveQuiz} className="space-y-12">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="space-y-3">
                            ...
                            <select
                                ...
                                className="w-full bg-gray-50 dark:bg-black/20 border border-gray-100 dark:border-white/10 rounded-2xl px-6 py-5 text-gray-900 dark:text-white font-black focus:ring-4 focus:ring-[#8b5cf6]/10 outline-none transition-all shadow-inner appearance-none uppercase tracking-widest text-[11px]"
                                required
                            >
                                <option value="">{t('admin.quizzes.modals.select_course')}</option>
                                {courses.map(c => (
                                    <option key={c.id} value={c.id} className="bg-white dark:bg-[#0d0d14]">{c.name} (S{c.semester})</option>
                                ))}
                            </select>
                        </div>
                        <div className="space-y-3">
                            ...
                            <input
                                ...
                                className="w-full bg-gray-50 dark:bg-black/20 border border-gray-100 dark:border-white/10 rounded-2xl px-6 py-5 text-gray-900 dark:text-white text-lg font-black focus:ring-4 focus:ring-[#8b5cf6]/10 outline-none transition-all shadow-inner uppercase"
                                required
                            />
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-2 gap-8">
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    <input type="number" value={quizForm.time_limit_minutes} onChange={(e) => setQuizForm({ ...quizForm, time_limit_minutes: e.target.value })} className="w-full bg-gray-50 dark:bg-black/20 border border-gray-100 dark:border-white/10 rounded-2xl px-6 py-5 text-gray-900 dark:text-white font-black outline-none focus:ring-4 focus:ring-[#8b5cf6]/10 transition-all shadow-inner" required />
                                    <Clock className="absolute inset-inline-end-5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#8b5cf6]" />
                                </div>
                            </div>
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    <input type="number" value={quizForm.passing_score} onChange={(e) => setQuizForm({ ...quizForm, passing_score: e.target.value })} className="w-full bg-gray-50 dark:bg-black/20 border border-gray-100 dark:border-white/10 rounded-2xl px-6 py-5 text-gray-900 dark:text-white font-black outline-none focus:ring-4 focus:ring-[#8b5cf6]/10 transition-all shadow-inner" required />
                                    <span className="absolute inset-inline-end-5 top-1/2 -translate-y-1/2 font-black text-[#8b5cf6]">%</span>
                                </div>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-8">
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    <input type="number" value={quizForm.max_attempts} onChange={(e) => setQuizForm({ ...quizForm, max_attempts: e.target.value })} className="w-full bg-gray-50 dark:bg-black/20 border border-gray-100 dark:border-white/10 rounded-2xl px-6 py-5 text-gray-900 dark:text-white font-black outline-none focus:ring-4 focus:ring-[#8b5cf6]/10 transition-all shadow-inner" />
                                    <RotateCcw className="absolute inset-inline-end-5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#8b5cf6]" />
                                </div>
                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 p-10 bg-gray-50 dark:bg-[#0c0c14]/40 border border-gray-100 dark:border-white/5 rounded-[2.5rem] shadow-inner relative overflow-hidden group">
                    {/* Inner shadow effect */}
                    ...
                    <div className="space-y-3 relative z-10">
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 flex items-center gap-3">
                            <Calendar className="w-4 h-4 text-[#8b5cf6]" /> {t('admin.quizzes.form.start_date')}
                        </label>
                        <input type="datetime-local" value={quizForm.start_date} onChange={(e) => setQuizForm({ ...quizForm, start_date: e.target.value })} className="w-full bg-white dark:bg-[#0d0d14] text-gray-900 dark:text-white border border-gray-100 dark:border-white/10 rounded-2xl px-6 py-5 font-black focus:ring-4 focus:ring-[#8b5cf6]/10 outline-none transition-all shadow-sm" />
                    </div>
                    <div className="space-y-3 relative z-10">
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 flex items-center gap-3">
                            <Calendar className="w-4 h-4 text-rose-500" /> {t('admin.quizzes.form.end_date')}
                        </label>
                        <input type="datetime-local" value={quizForm.end_date} onChange={(e) => setQuizForm({ ...quizForm, end_date: e.target.value })} className="w-full bg-white dark:bg-[#0d0d14] text-gray-900 dark:text-white border border-gray-100 dark:border-white/10 rounded-2xl px-6 py-5 font-black focus:ring-4 focus:ring-rose-500/10 outline-none transition-all shadow-sm" />
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    ...
                    <textarea placeholder={t('admin.quizzes.modals.placeholder_desc')} value={quizForm.description} onChange={(e) => setQuizForm({ ...quizForm, description: e.target.value })} className="w-full bg-gray-50 dark:bg-black/20 border border-gray-100 dark:border-white/10 rounded-[2rem] p-8 text-gray-900 dark:text-white font-bold focus:ring-4 focus:ring-[#8b5cf6]/10 outline-none transition-all shadow-inner resize-none min-h-[140px]" rows="3" />
                </div>

                <div className="flex gap-6 pt-10 border-t border-gray-100 dark:border-white/5">
                    <button type="submit" disabled={loading} className="flex-1 bg-[#8b5cf6] hover:bg-[#7c3aed] text-white font-black py-5 rounded-[2.5rem] shadow-xl shadow-purple-500/20 transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3">

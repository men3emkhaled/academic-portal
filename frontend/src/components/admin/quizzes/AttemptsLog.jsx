...
const AttemptsLog = ({ attempts, selectedQuiz }) => {
  ...
  return (
    <div className="bg-white dark:bg-[#0d0d14] border border-gray-100 dark:border-white/5 rounded-[3rem] overflow-hidden shadow-inner animate-in fade-in duration-700">
      <div className="overflow-x-auto">
        <table className="w-full text-start border-collapse">
            <thead>
            <tr className="bg-gray-50/50 dark:bg-white/[0.02]">
                <th className="py-8 px-10 text-[10px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-[0.3em] text-start">{t('admin.quizzes.attempts.student_col')}</th>
                <th className="py-8 px-8 text-[10px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-[0.3em] text-start">{t('admin.quizzes.attempts.date_col')}</th>
                <th className="py-8 px-8 text-[10px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-[0.3em] text-start">{t('admin.quizzes.attempts.score_col')}</th>
                <th className="py-8 px-10 text-[10px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-[0.3em] text-end">{t('admin.quizzes.attempts.status_col')}</th>
            </tr>
            </thead>
                ...
                <tr key={att.id} className="group hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-all duration-500">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="w-14 h-14 rounded-[1.25rem] bg-gray-100 dark:bg-black border border-gray-100 dark:border-white/10 flex items-center justify-center overflow-hidden shadow-inner group-hover:scale-105 transition-transform duration-500">
                            {att.avatar_url ? (
                                ...
                                <User className="w-7 h-7 text-gray-300 dark:text-slate-500" />
                            )}
                        </div>
                        <div>
                            ...
                            <p className="text-[9px] text-gray-400 dark:text-slate-400 font-black tracking-[0.3em] uppercase mt-2 italic">ID_REF: {att.student_id}</p>
                        </div>
                    </div>
                    </td>
                    <td className="py-8 px-8 text-start">
                    <div className="space-y-2">
                        <div className="flex items-center gap-3 text-[10px] font-black text-gray-500 dark:text-gray-300 uppercase tracking-widest">
                            <Clock className="w-4 h-4 text-[#8b5cf6]" /> {new Date(att.started_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </div>
                        <div className="flex items-center gap-3 text-[10px] font-black text-gray-400 dark:text-white/20 uppercase tracking-widest italic">
                            <Calendar className="w-4 h-4 text-[#8b5cf6]/40" /> {new Date(att.started_at).toLocaleDateString()}
                        </div>
                    </div>
                    </td>
                    <td className="py-8 px-8 text-start">
                    <div className="flex items-center gap-6">
                        <div className="flex flex-col">
                            ...
                            <span className="text-[8px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest mt-1 italic">{t('admin.quizzes.attempts.raw_points')}</span>
                        </div>
                        <div className="w-32 h-2.5 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden shadow-inner relative">
                            <div className={`h-full rounded-full transition-all duration-1000 ${
                                isPassed ? 'bg-[#2cfc7d] shadow-[0_0_12px_rgba(44,252,125,0.4)]' : 'bg-rose-500 shadow-[0_0_12px_rgba(244,63,94,0.4)]'
                            }`} style={{ width: `${att.percentage || 0}%` }}></div>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className={`text-base font-black tracking-tighter ${isPassed ? 'text-[#2cfc7d]' : 'text-rose-500'}`}>

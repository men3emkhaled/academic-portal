...
const QuestionBank = ({ questions, setShowQuestionForm, editQuestion, handleDeleteQuestion }) => {
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pb-10 border-b border-gray-100 dark:border-white/5">
          <div className="flex items-center gap-4">
              <div className="w-3 h-3 bg-primary rounded-full animate-pulse shadow-[0_0_12px_rgba(46,204,113,0.5)]"></div>
          ...
          <button
              ...
              className="group px-10 py-4.5 bg-gray-900 dark:bg-white text-white dark:text-black rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-3 shadow-xl hover:scale-105 active:scale-95"
          >
      ...
      {questions.length === 0 ? (
          <div className="bg-gray-50 dark:bg-white/[0.01] border-2 border-dashed border-gray-200 dark:border-white/5 rounded-[3rem] py-40 text-center flex flex-col items-center group shadow-inner">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {questions.map((q, idx) => (
            <div key={q.id} className="group relative bg-white dark:bg-white/[0.01] border border-gray-100 dark:border-white/5 rounded-[2.5rem] p-10 hover:border-primary/30 transition-all duration-500 shadow-sm hover:shadow-2xl hover:shadow-primary/5 text-start">
               <div className="flex justify-between items-start mb-8 relative z-10">
                  <div className="flex items-center gap-4">
                      <span className="w-12 h-12 rounded-[1.25rem] bg-primary/5 border border-primary/20 flex items-center justify-center text-sm font-black text-primary dark:text-primary shadow-inner">{idx + 1}</span>
                      <div className="flex flex-col">
                        <span className="text-[10px] font-black text-primary/60 dark:text-primary/60 uppercase tracking-widest mb-1">{t('admin.quizzes.questions.points_label', { count: q.points })}</span>
               ...
               {q.image_url && (
                  <div className="mb-8 rounded-[2rem] overflow-hidden border border-gray-100 dark:border-white/10 group-hover:border-primary/20 transition-all duration-700 shadow-sm relative">
                      ...
                      {q.options.map((opt, i) => {
                          ...
                          return (
                              <div key={i} className={`flex items-center gap-5 px-8 py-5 rounded-[1.5rem] border transition-all duration-500 ${
                                  ...
                                  : 'bg-gray-50/50 dark:bg-white/[0.02] border-gray-100 dark:border-white/5 text-gray-500 dark:text-slate-500'
                              }`}>
                                  <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black transition-all ${isCorrect ? 'bg-emerald-500 text-white' : 'bg-gray-200 dark:bg-white/10'}`}>
                                    {letter}
                                  </div>
               ...
               {q.explanation && (
                  <div className="mt-10 pt-8 border-t border-gray-100 dark:border-white/5 flex gap-5 italic relative z-10">
                      <div className="w-10 h-10 bg-primary/10 rounded-[1.25rem] flex items-center justify-center flex-shrink-0 text-primary border border-primary/20">
                        <HelpCircle className="w-5 h-5" />
                      </div>
                      <p className="text-[11px] font-bold text-gray-400 dark:text-slate-500 leading-relaxed">{q.explanation}</p>
                  </div>
               )}
            </div>
          ))}

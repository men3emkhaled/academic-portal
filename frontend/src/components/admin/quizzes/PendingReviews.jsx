...
const PendingReviews = ({ quizId = null }) => {
  ...
  const handleSubmitGrade = async (answerId, maxPoints) => {
    ...
    try {
      ...
      if (allGraded) {
        toast.success('All answers graded for this attempt');
    ...
    <div className="animate-in fade-in duration-700 pb-10">
      {loading ? (
        <div className="flex flex-col items-center justify-center py-48 opacity-50">
           ...
           <p className="text-xs font-black uppercase tracking-widest text-gray-500">Scanning for attempts...</p>
        </div>
        ...
        <div className="bg-white/80 dark:bg-black/20 border-2 border-dashed border-gray-200 dark:border-white/10 rounded-[3rem] py-48 text-center flex flex-col items-center group transition-all duration-500">
            ...
            <h4 className="text-xl font-black uppercase tracking-[0.3em] text-gray-900 dark:text-white">All Clear!</h4>
            <p className="text-sm font-bold mt-4 tracking-widest text-gray-500">No written answers are waiting for review in this quiz.</p>
        </div>
      ) : (
          ...
          <div className="xl:col-span-1 space-y-4 max-h-[800px] overflow-y-auto pr-3 no-scrollbar">
            <div className="flex items-center justify-between mb-4 px-4 py-2 bg-amber-500/5 border border-amber-500/10 rounded-2xl">
                <span className="text-[10px] font-black text-amber-600 uppercase tracking-widest flex items-center gap-2">
                    <Timer className="w-3.5 h-3.5" /> High Priority
                </span>
          ...
          <div className="xl:col-span-3">
            {!selectedAttempt ? (
              <div className="bg-white/80 dark:bg-[#111]/80 backdrop-blur-xl border-2 border-dashed border-gray-200 dark:border-white/10 rounded-[3rem] py-56 text-center flex flex-col items-center group transition-all duration-500 shadow-sm">
                  ...
                  <p className="text-sm font-bold mt-4 tracking-widest text-gray-500 max-w-xs">Select an attempt from the queue to start manual validation.</p>
              </div>
            ) : detailsLoading ? (
               <div className="bg-white/80 dark:bg-[#111]/80 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-[3rem] py-60 flex flex-col items-center justify-center shadow-sm">
                  <Activity className="w-12 h-12 text-emerald-600 animate-spin mb-6" />
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Hydrating Module Data...</p>
               </div>
            ) : (
                  ...
                  <div className="flex items-center gap-8">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-4 mb-2">
                                ...
                                <span className="px-3 py-1 bg-amber-500/10 text-amber-600 border border-amber-500/20 rounded-xl text-[10px] font-black uppercase tracking-widest">Awaiting Grade</span>
                            </div>
                ...
                <div className="p-10 space-y-12 max-h-[850px] overflow-y-auto no-scrollbar">
                  {reviewAnswers.length === 0 ? (
                    <div className="text-center py-32 opacity-30 grayscale">
                        ...
                        <p className="text-sm font-black uppercase tracking-widest text-gray-500">No answer packets found</p>
                    </div>
                  ) : (
                    reviewAnswers.map((ans, idx) => (
                      <div key={ans.answer_id} className="group relative bg-white/50 dark:bg-black/20 border border-gray-100 dark:border-white/10 rounded-[3rem] p-10 hover:border-emerald-500/40 transition-all duration-500 shadow-sm hover:shadow-xl">
                           <div className="flex items-center gap-5 mb-10 pb-6 border-b border-gray-100 dark:border-white/10">
                                ...
                                <div className="flex-1">
                                    <h5 className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Question Text</h5>
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-10">
                                <div className="space-y-4">
                                    <h5 className="text-[10px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-2">
                                        <Edit3 className="w-4 h-4" /> Written Answer
                                    </h5>
                                    <div className="p-8 rounded-[2rem] bg-gray-50/50 dark:bg-black/40 border border-gray-100 dark:border-white/5 min-h-[160px] shadow-inner">
                                        {ans.student_answer ? (
                                            ...
                                            <p className="text-rose-500/50 italic text-xs uppercase tracking-widest font-black">No response provided</p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <h5 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest flex items-center gap-2">
                                        <ImageIcon className="w-4 h-4" /> Student Photo
                                    </h5>
                                    {ans.written_answer_url ? (
                                        ...
                                        <div className="w-full aspect-video rounded-[2rem] bg-gray-50 dark:bg-black/40 border-2 border-dashed border-gray-200 dark:border-white/5 flex flex-col items-center justify-center opacity-40">
                                            ...
                                            <p className="text-[10px] font-black uppercase text-gray-400">No image uploaded</p>
                                        </div>
                                    )}
                                </div>
                           </div>
                           ...
                           <div className="pt-8 border-t border-gray-100 dark:border-white/10 flex flex-wrap items-center justify-between gap-6">
                              {ans.graded ? (
                                <div className="flex items-center gap-4 px-8 py-4 rounded-[1.5rem] bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 animate-in zoom-in-95">
                                  ...
                                  <span className="text-sm font-black uppercase tracking-widest">Graded: {ans.points_earned} / {ans.points} Points</span>
                                </div>
                              ) : (
                              ...
                              <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                 <Database className="w-3.5 h-3.5 text-gray-300" /> Reference: #{ans.answer_id}
                              </div>
                           </div>
                      </div>
                    ))

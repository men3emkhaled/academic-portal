    ...
    <div className="animate-in fade-in duration-700 pb-10 text-start">
      {loading ? (
        <div className="flex flex-col items-center justify-center py-48 opacity-50">
           ...
           <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-500">{t('admin.quizzes.registry_stream')}</p>
        </div>
      ) : filteredAttempts.length === 0 ? (
        <div className="bg-white dark:bg-white/[0.01] border-2 border-dashed border-gray-100 dark:border-white/5 rounded-[3.5rem] py-48 text-center flex flex-col items-center group shadow-inner">
            <div className="w-24 h-24 bg-primary/5 dark:bg-primary/10 rounded-full flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-700">
          ...
          <div className="xl:col-span-1 space-y-6 max-h-[850px] overflow-y-auto pr-3 no-scrollbar">
            ...
            {filteredAttempts.map((attempt) => (
              ...
              >
                  ...
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="w-14 h-14 rounded-[1.25rem] bg-gray-100 dark:bg-black border border-gray-100 dark:border-white/10 flex items-center justify-center overflow-hidden shrink-0 shadow-inner group-hover:scale-105 transition-transform">
                      {attempt.avatar_url ? (
                        ...
                        <User className={`w-7 h-7 ${selectedAttempt?.attempt_id === attempt.attempt_id ? 'text-white/50' : 'text-gray-300'}`} />
                      )}
                    </div>
                    <div className="min-w-0">
                      ...
                      <p className={`text-[9px] font-black tracking-[0.3em] uppercase mt-2 italic ${selectedAttempt?.attempt_id === attempt.attempt_id ? 'text-white/60' : 'text-gray-400'}`}>ID: {attempt.student_id}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className={`flex items-center justify-between text-[9px] font-black uppercase tracking-[0.2em] ${selectedAttempt?.attempt_id === attempt.attempt_id ? 'text-white/50' : 'text-gray-400'}`}>
          ...
          <div className="xl:col-span-3">
            {!selectedAttempt ? (
              <div className="bg-white dark:bg-white/[0.01] border-2 border-dashed border-gray-100 dark:border-white/5 rounded-[4rem] py-60 text-center flex flex-col items-center group transition-all duration-700 h-full justify-center shadow-inner">
               ...
               <div className="bg-white dark:bg-white/[0.01] border border-gray-100 dark:border-white/5 rounded-[4rem] py-60 flex flex-col items-center justify-center h-full shadow-inner animate-pulse">
                  <Activity className="w-16 h-16 text-primary animate-spin mb-8" />
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.4em]">{t('admin.quizzes.registry_stream')}</p>
               </div>
            ) : (
              <div className="bg-white dark:bg-white/[0.01] border border-gray-100 dark:border-white/5 rounded-[4rem] overflow-hidden shadow-sm animate-in fade-in slide-in-from-right-12 duration-700">
                <div className="p-12 border-b border-gray-100 dark:border-white/5 bg-gray-50/30 dark:bg-white/[0.01]">
                  <div className="flex items-center gap-10">
                        <div className="w-24 h-24 rounded-[2.5rem] bg-primary/5 border-2 border-primary/20 flex items-center justify-center overflow-hidden shadow-inner scale-110">
                            {selectedAttempt.avatar_url ? (
                              ...
                              <User className="w-12 h-12 text-primary opacity-30" />
                            )}
                        </div>
                        <div className="flex-1">
                            ...
                            <div className="flex items-center gap-8">
                                <div className="flex items-center gap-3 text-xs font-black text-gray-400 uppercase tracking-[0.2em] italic">
                                    <Hash className="w-5 h-5 text-primary opacity-40" /> ID_{selectedAttempt.student_id}
                                </div>
                                <div className="w-2 h-2 bg-primary/20 rounded-full"></div>
                                <div className="flex items-center gap-3 text-xs font-black text-primary uppercase tracking-[0.2em] italic leading-none">
                ...
                <div className="p-12 space-y-16 max-h-[900px] overflow-y-auto custom-scrollbar">
                  {reviewAnswers.length === 0 ? (
                    ...
                    reviewAnswers.map((ans, idx) => (
                      <div key={ans.answer_id} className="group relative bg-white dark:bg-white/[0.01] border border-gray-100 dark:border-white/5 rounded-[3.5rem] p-12 hover:border-primary/40 transition-all duration-700 shadow-sm hover:shadow-2xl text-start">
                           <div className="flex items-start gap-8 mb-12 pb-10 border-b border-gray-100 dark:border-white/5">
                                <span className="w-14 h-14 rounded-[1.5rem] bg-primary/10 border border-primary/20 flex items-center justify-center text-base font-black text-primary shadow-inner shrink-0">{idx + 1}</span>
                                <div className="flex-1">
                                    <h5 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] mb-3">{t('admin.quizzes.authorized_entities')}</h5>
                                ...
                                <span className="px-6 py-2.5 rounded-2xl bg-primary/5 text-primary border border-primary/20 text-[10px] font-black uppercase tracking-widest shadow-sm">
                                    {ans.points} PTS
                                </span>
                           </div>
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
                                <div className="space-y-6">
                                    ...
                                    <div className="p-10 rounded-[2.5rem] bg-gray-50 dark:bg-black/20 border border-gray-100 dark:border-white/5 min-h-[220px] shadow-inner group-hover:bg-primary/5 transition-colors duration-700">
                                        {ans.student_answer ? (
                           ...
                           <div className="pt-10 border-t border-gray-100 dark:border-white/10 flex flex-wrap items-center justify-between gap-8">
                              {ans.graded ? (
                                <div className="flex items-center gap-6 px-10 py-5 rounded-[2rem] bg-primary text-white shadow-2xl shadow-primary/30 animate-in zoom-in duration-500 scale-105">
                                ...
                                <div className="flex items-center gap-6">
                                  <div className="relative group/input">
                                      <input
                                        ...
                                        className="w-44 bg-gray-50 dark:bg-black border border-gray-100 dark:border-white/10 rounded-2xl px-8 py-5 text-gray-900 dark:text-white text-2xl font-black focus:ring-4 focus:ring-primary/10 outline-none transition-all shadow-inner"
                                      />
                                      <Award className="absolute inset-inline-end-6 top-1/2 -translate-y-1/2 w-6 h-6 text-primary group-focus-within/input:scale-125 transition-transform" />
                                  </div>
                                  <button

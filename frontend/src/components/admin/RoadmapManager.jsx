    ...
    <div className="animate-fadeIn pb-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-black text-white tracking-tight flex items-center gap-3">
              <Map className="w-6 h-6 text-indigo-400" /> Roadmap Engine
            </h2>
            <p className="text-slate-500 text-xs font-bold uppercase tracking-[0.2em] mt-1">Career Tracks & Milestone Architecture</p>
          </div>
        </div>
        <button
        ...
        <div className="admin-card relative overflow-hidden group">
            ...
            <div className="relative z-10">
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <div>
                        <h3 className="text-lg font-black text-white tracking-tight">Active Learning Tracks</h3>
                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Root node configuration</p>
                    </div>
                </div>
                ...
                {loading && tracks.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {tracks.map((track) => (
                            <div
                                key={track.id}
                                className={`group/track relative p-6 rounded-[2rem] border transition-all duration-500 cursor-pointer ${
                                    selectedTrack?.id === track.id
                                        ? 'bg-indigo-500/10 border-indigo-500 shadow-[0_20px_40px_rgba(99,102,241,0.1)]'
                                        : 'bg-white/5 border-white/5 hover:border-white/20 hover:bg-white/[0.04]'
                                }`}
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    <div className="min-w-0">
                                        <div className="flex items-center gap-2">
                                            <h4 className="font-black text-white tracking-tight truncate group-hover/track:text-indigo-400 transition-colors uppercase text-sm">{track.name}</h4>
                                    ...
                                    <div className="flex gap-1.5 opacity-0 group-hover/track:opacity-100 transition-all">
                                        <button onClick={(e) => { e.stopPropagation(); editTrack(track); }} className="w-8 h-8 flex items-center justify-center rounded-lg bg-white/5 text-yellow-500 hover:bg-yellow-500 hover:text-black transition-all"><Edit3 className="w-3.5 h-3.5" /></button>
                                        <button onClick={(e) => { e.stopPropagation(); handleDeleteTrack(track); }} className="w-8 h-8 flex items-center justify-center rounded-lg bg-red-400/10 text-red-400 hover:bg-red-400 hover:text-white transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                                    </div>
                                </div>
                                <p className="text-[10px] text-slate-500 font-medium leading-relaxed line-clamp-2">{track.description || 'No definition set for this node.'}</p>
                        ...
                        {tracks.length === 0 && (
                            <div className="col-span-full border border-dashed border-white/5 rounded-[2rem] py-12 text-center grayscale opacity-20">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-sm font-black text-white uppercase tracking-[0.1em] flex items-center gap-2">
                                Task Matrix for <span className="text-indigo-400">{selectedTrack.name}</span>
                            </h3>
                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5 truncate max-w-[300px]">{selectedTrack.description}</p>
                        </div>
                    </div>
                    <button 
                ...
                <div className="p-8">
                    {tasks.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {tasks.map((task) => (
                                <div key={task.id} className="group/task relative bg-white/[0.02] border border-white/5 rounded-3xl p-6 hover:bg-white/[0.04] hover:border-indigo-500/20 transition-all duration-300">
                                    <div className="flex justify-between items-start gap-4">
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-3 mb-3">
                                                <span className="flex-shrink-0 w-8 h-8 rounded-lg bg-slate-900 border border-white/5 flex items-center justify-center text-[10px] font-black text-indigo-400 group-hover/task:bg-indigo-500/20 transition-all lowercase">0x{task.order_index < 10 ? '0' : ''}{task.order_index}</span>
                                                <h4 className="font-black text-white tracking-tight truncate text-sm">{task.title}</h4>
                                            </div>
                                            {task.description && <p className="text-[10px] text-slate-500 font-medium leading-relaxed line-clamp-2">{task.description}</p>}
                                        </div>
                                        <div className="flex gap-1.5 opacity-0 group-hover/task:opacity-100 transition-all">
                                            <button onClick={() => editTask(task)} className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 text-yellow-500 hover:bg-yellow-500 hover:text-black transition-all"><Edit3 className="w-4 h-4" /></button>
                                            <button onClick={() => handleDeleteTask(task)} className="w-9 h-9 flex items-center justify-center rounded-xl bg-red-400/10 text-red-400 hover:bg-red-400 hover:text-white transition-all"><Trash2 className="w-4 h-4" /></button>
                                        </div>
                                    </div>
      ...
      {showTrackForm && (
        <div className="admin-modal-backdrop" onClick={resetTrackForm}>
          <div className="bg-[#111111] border border-white/10 rounded-[2.5rem] p-12 w-full max-w-lg shadow-2xl relative overflow-hidden animate-fadeInUp" onClick={e => e.stopPropagation()}>
            <div className="absolute -top-32 -right-32 w-80 h-80 bg-indigo-500/10 blur-[100px] rounded-full"></div>
            
            <div className="relative z-10">
                <div className="flex items-center gap-5 mb-10">
                    <div className="w-14 h-14 bg-indigo-500/20 rounded-2xl flex items-center justify-center border border-indigo-500/30 text-indigo-400">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Career Pathway Definition Interface</p>
                    </div>
                </div>

                <form onSubmit={handleSaveTrack} className="space-y-6">
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Node Identity *</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Logical Rationale</label>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group w-fit">
                        <div className={`w-10 h-6 rounded-full p-1 transition-all duration-300 ${trackForm.is_primary ? 'bg-indigo-500' : 'bg-slate-800'}`}>
                        ...
                        <span className="text-[10px] font-black uppercase text-slate-400 group-hover:text-white transition-colors tracking-widest flex items-center gap-2">
                            <Star className={`w-3.5 h-3.5 ${trackForm.is_primary ? 'text-yellow-500 fill-yellow-500' : 'text-slate-600'}`} /> SET_AS_PRIMARY_NODE
                        </span>
      ...
      {showTaskForm && selectedTrack && (
        <div className="admin-modal-backdrop" onClick={resetTaskForm}>
          <div className="bg-[#111111] border border-white/10 rounded-[2.5rem] p-12 w-full max-w-lg shadow-2xl relative overflow-hidden animate-fadeInUp" onClick={e => e.stopPropagation()}>
            ...
            <div className="relative z-10">
                <div className="flex items-center gap-5 mb-10">
                    <div className="w-14 h-14 bg-emerald-500/20 rounded-2xl flex items-center justify-center border border-emerald-500/30 text-emerald-400">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Linked to: {selectedTrack.name}</p>
                    </div>
                </div>

                <form onSubmit={handleSaveTask} className="space-y-6">
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Milestone Label *</label>
                        <input type="text" placeholder="e.g. Distributed Consensus IV" value={taskForm.title} onChange={(e) => setTrackForm({ ...taskForm, title: e.target.value })} className="admin-input" required />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div className="md:col-span-1 space-y-2">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Sequence</label>
                        ...
                        <div className="md:col-span-3 space-y-2">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Operational Rationale</label>
                    ...
                    <div className="flex gap-4 pt-6">
                        <button type="submit" disabled={loading} className="flex-1 admin-btn-primary h-[65px] bg-emerald-600 border-none font-black uppercase tracking-widest">

    ...
    <div className="space-y-12 animate-in fade-in duration-700 pb-10 text-start">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 flex items-center gap-8 bg-white dark:bg-white/[0.01] border border-gray-100 dark:border-white/5 p-10 rounded-[3rem] shadow-sm relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-gray-400 text-[11px] font-black mt-3 uppercase tracking-widest italic opacity-60">{t('admin.records.description')}</p>
          </div>
        </div>
              ...
              <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-12">
                  <div className="w-full lg:w-[450px] space-y-8">
                      <div className="space-y-4">
                          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4 italic">{t('admin.records.search_label')}</label>
                          <div className="relative group/input">
                              <Search className="absolute inset-inline-start-6 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-hover/input:text-primary transition-colors" />
                              <input 
                                  type="text"
                                  placeholder={t('admin.records.search_placeholder')}
                                  value={searchTerm}
                                  onChange={(e) => setSearchTerm(e.target.value)}
                                  className="w-full bg-gray-50 dark:bg-black/40 border border-gray-100 dark:border-white/10 rounded-2xl px-14 py-5 font-black text-[13px] focus:ring-4 focus:ring-primary/10 outline-none transition-all shadow-inner"
                              />
                          </div>
                      </div>
                      <div className="space-y-4">
                          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4 italic">{t('admin.records.select_label')}</label>
                          <div className="relative">
                              <Users className="absolute inset-inline-start-6 top-1/2 -translate-y-1/2 w-5 h-5 text-primary" />
                              <select
                                  value={selectedStudent?.id || ''}
                                  onChange={(e) => handleSelectStudent(e.target.value)}
                                  className="w-full bg-gray-50 dark:bg-black/40 border border-gray-100 dark:border-white/10 rounded-2xl px-14 py-5 font-black text-[13px] text-gray-900 dark:text-white focus:ring-4 focus:ring-primary/10 outline-none transition-all appearance-none shadow-inner uppercase tracking-widest"
                              >
                  ...
                  {selectedStudent ? (
                      <div className="flex-1 flex flex-col sm:flex-row items-center gap-10 bg-primary/5 border border-primary/10 rounded-[3rem] p-8 animate-in fade-in zoom-in duration-700 shadow-sm relative overflow-hidden group/card">
                          <div className="absolute top-0 right-0 w-32 h-32 bg-primary/10 blur-3xl rounded-full -translate-y-1/2 translate-x-1/2 group-hover/card:scale-150 transition-transform duration-1000"></div>
                          ...
                          <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-8 w-full relative z-10">
                              <div className="space-y-2">
                                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">{t('admin.records.name_label')}</p>
                                  <p className="text-gray-900 dark:text-white font-black tracking-tighter text-xl leading-tight uppercase group-hover/card:text-primary transition-colors">{selectedStudent.name}</p>
                              </div>
                              <div className="space-y-2">
                                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">{t('admin.records.level_label')}</p>
                                  <span className="inline-flex items-center px-4 py-2 rounded-xl bg-primary text-white text-[10px] font-black uppercase tracking-widest shadow-lg shadow-primary/20">
                                      LVL {selectedStudent.level}
                                  </span>
                              </div>
                              <div className="space-y-2">
                                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">{t('admin.records.section_label')}</p>
                                  <p className="text-gray-900 dark:text-white font-black tracking-tighter text-xl uppercase">{selectedStudent.section || '—'}</p>
                              </div>
                              <div className="space-y-2">
                                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">{t('admin.records.id_label')}</p>
                                  <p className="text-primary font-black tracking-widest text-sm flex items-center gap-2">
      ...
      {selectedStudent && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-10 animate-in slide-in-from-bottom-8 duration-1000">
          {/* Course Enrollment */}
          <div className="bg-white dark:bg-white/[0.01] border border-gray-100 dark:border-white/5 rounded-[3.5rem] overflow-hidden shadow-sm relative group">
              <div className="p-10 border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/[0.02] flex justify-between items-center relative z-10">
                  ...
                  <button 
                      ...
                      className="px-6 py-4 bg-primary hover:bg-black dark:hover:bg-white dark:hover:text-black text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-95 shadow-xl shadow-primary/20 flex items-center gap-3 group/btn"
                  >
                  ...
                  <table className="w-full text-start border-collapse">
                      <thead>
                          <tr className="bg-gray-50/50 dark:bg-white/[0.01]">
                              <th className="py-6 px-10 text-[10px] font-black text-gray-400 uppercase tracking-widest text-start">{t('admin.records.course_name_col')}</th>
                              <th className="py-6 px-6 text-[10px] font-black text-gray-400 uppercase tracking-widest text-start">{t('admin.records.semester_col')}</th>
                              <th className="py-6 px-10 text-[10px] font-black text-gray-400 uppercase tracking-widest text-end">{t('admin.records.actions_col')}</th>
                          </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                          {studentCourses.length === 0 ? (
                              ...
                              studentCourses.map((sc) => (
                                  <tr key={sc.id} className="group/item hover:bg-primary/5 transition-all duration-500">
                                      <td className="py-8 px-10">
                                          <div className="flex items-center gap-5">
                                              <div className="w-12 h-12 rounded-2xl bg-white dark:bg-black border border-gray-100 dark:border-white/10 flex items-center justify-center text-primary shadow-inner group-hover/item:bg-primary group-hover/item:text-white transition-all duration-500">
                                                  <CheckCircle className="w-5 h-5" />
                                              </div>
                                              <p className="text-gray-900 dark:text-white font-black tracking-tighter text-lg group-hover/item:text-primary transition-colors uppercase">{sc.course_name}</p>
                                          </div>
                                      </td>
                                      <td className="py-8 px-6">
                                          <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest bg-gray-100 dark:bg-white/10 border border-gray-200 dark:border-white/10 px-4 py-2 rounded-xl shadow-inner">
              ...
              <div className="overflow-x-auto relative z-10 custom-scrollbar">
                  <table className="w-full text-start border-collapse">
                      <thead>
                          <tr className="bg-gray-50/50 dark:bg-white/[0.01]">
                              <th className="py-6 px-10 text-[10px] font-black text-gray-400 uppercase tracking-widest text-start">{t('admin.records.course_name_col')}</th>
                              <th className="py-6 px-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">{t('admin.records.midterm_col')}</th>
                              <th className="py-6 px-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">{t('admin.records.practical_col')}</th>
                              <th className="py-6 px-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">{t('admin.records.oral_col')}</th>
                              <th className="py-6 px-10 text-[10px] font-black text-gray-400 uppercase tracking-widest text-end">{t('admin.records.actions_col')}</th>
                          </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                          {studentGrades.length === 0 ? (
                              ...
                              studentGrades.map((grade, idx) => (
                                  <tr key={idx} className="group/grade hover:bg-amber-500/5 transition-all duration-500">
                                      ...
                                      <td className="py-8 px-4 text-center">
                                          <div className={`w-12 h-12 rounded-2xl mx-auto flex items-center justify-center text-sm font-black border transition-all duration-500 ${
                                              grade.practical_score ? 'bg-primary/10 border-primary/30 text-primary shadow-inner group-hover/grade:scale-110 group-hover/grade:-rotate-6' : 'bg-gray-50/50 dark:bg-white/5 border-gray-100 dark:border-white/5 text-gray-300'
                                          }`}>
      ...
      {showAddCourseModal && selectedStudent && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-6 animate-in fade-in duration-300" onClick={() => setShowAddCourseModal(false)}>
          <div className="bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-white/10 rounded-[3.5rem] p-10 lg:p-12 w-full max-w-xl shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-500" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative z-10">
                <div className="flex items-center gap-6 mb-10 pb-8 border-b border-gray-100 dark:border-white/5">
                    <div className="w-16 h-16 bg-primary/10 rounded-[1.5rem] flex items-center justify-center border border-primary/20 text-primary shadow-inner">
                ...
                <div className="space-y-10">
                    <div className="space-y-4">
                        ...
                        <div className="relative">
                            ...
                            <ChevronRight className={`absolute inset-inline-end-8 top-1/2 -translate-y-1/2 pointer-events-none text-primary rotate-90 ${i18n.language === 'ar' ? 'rotate-[-90deg]' : ''}`} />
                        </div>
                    </div>
                    <div className="flex gap-4 pt-6 border-t border-gray-100 dark:border-white/5">
                        <button onClick={handleAddCourseToStudent} className="flex-1 bg-primary text-white font-black py-6 rounded-[2rem] shadow-2xl shadow-primary/30 transition-all hover:scale-[1.02] active:scale-95 uppercase tracking-[0.3em] text-[11px] flex items-center justify-center gap-3">
      ...
      {showGradeModal && editingGrade && selectedStudent && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-6 animate-in fade-in duration-300" onClick={() => setShowGradeModal(false)}>
          <div className="bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-white/10 rounded-[3.5rem] p-10 lg:p-12 w-full max-w-2xl shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-500" onClick={(e) => e.stopPropagation()}>
                                ...
                                <button
                                    ...
                                    className={`py-6 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest border transition-all duration-500 flex flex-col items-center gap-3 ${
                                        editGradeForm.status === status 
                                            ? 'bg-primary/10 border-primary/50 text-primary shadow-xl scale-105' 
                                            : 'bg-gray-50/50 dark:bg-white/5 border-gray-100 dark:border-white/10 text-gray-400 hover:border-primary/30'
                                    }`}
                                >

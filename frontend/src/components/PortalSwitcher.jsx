...
import { GraduationCap, Briefcase, Shield, Sun, Moon, Languages, Home, ChevronDown, UserCheck, Users } from 'lucide-react';
...
const PortalSwitcher = () => {
  ...
  const getActivePortal = () => {
    ...
    if (location.pathname.startsWith('/doctor') || location.pathname.startsWith('/ta')) return 'instructor';
      ...
      >
        ...
        <div className="flex items-center gap-1 relative">
          {portals.map((portal) => {
            ...
            if (portal.id === 'instructor') {
              return (
                <div key={portal.id} ref={instructorRef} className="relative">
                  <button
                  ...
                  >
                    ...
                    <ChevronDown className={`w-3 h-3 sm:w-3.5 sm:h-3.5 transition-transform duration-300 ${instructorOpen ? 'rotate-180' : ''}`} />
                  </button>
                  ...
                  <AnimatePresence>
                    {instructorOpen && (
                      <motion.div
                        ...
                        className={`absolute top-full left-1/2 -translate-x-1/2 mt-2 min-w-[180px] rounded-xl border p-1.5 backdrop-blur-md shadow-2xl z-50 ${
                          isDarkMode ? 'bg-black/80 border-white/10' : 'bg-white/90 border-gray-200/30'
                        }`}
                      >
                        <button
                          onClick={() => { navigate('/doctor/login'); setInstructorOpen(false); }}
                          className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all duration-200 ${
                        ...
                        <button
                          onClick={() => { navigate('/ta/login'); setInstructorOpen(false); }}
                          className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all duration-200 ${
                            isDarkMode ? 'text-white/50 hover:text-white hover:bg-white/5' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'
                          }`}
                        >
        ...
        <div className={`w-[2px] h-7 mx-2 sm:mx-3 transition-colors ${isDarkMode ? 'bg-white/20' : 'bg-gray-300'}`} />

...
const VerifyEmail = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-dark text-gray-900 dark:text-white font-body p-4 relative overflow-hidden transition-colors duration-300">
      <div className="absolute top-[10%] left-[20%] w-[30vw] h-[30vw] bg-primary/10 dark:bg-primary/5 rounded-full pointer-events-none"></div>
      <div className="absolute bottom-[10%] right-[10%] w-[40vw] h-[40vw] bg-primary/15 dark:bg-primary/10 rounded-full pointer-events-none"></div>
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="bg-white/90 dark:bg-dark-card/80 backdrop-blur-sm border border-gray-200 dark:border-white/10 rounded-[2rem] p-8 shadow-xl dark:shadow-2xl transition-colors duration-300">
          <div className="flex flex-col items-center mb-8 text-center">
            ...
            <h2 className="text-2xl font-extrabold tracking-tight text-gray-900 dark:text-white mb-2">{t('verify_email.title')}</h2>
            <p className="text-gray-500 dark:text-gray-400 text-sm">{t('verify_email.subtitle')}</p>
          </div>
          ...
          {success ? (
            <div className="text-center py-6">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{t('verify_email.verified_title')}</h3>
              <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">{t('verify_email.verified_subtitle')}</p>
              <button
              ...
              >
                {t('verify_email.go_to_login')} <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              <div className="flex flex-col gap-2">
                <label className="text-xs uppercase tracking-widest text-green-500 dark:text-green-400 font-bold px-1" htmlFor="verify-id">{t('verify_email.student_id_label')}</label>

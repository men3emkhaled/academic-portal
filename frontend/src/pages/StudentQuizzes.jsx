      ...
      <div className="md:ml-64 pb-24 md:pb-8">
        <main className="pt-8 px-6 max-w-screen-xl mx-auto">
          ...
          <div className="mb-8 overflow-x-auto no-scrollbar">
            <div className="flex gap-2 bg-dark-glass p-2 rounded-2xl border border-white/10 shadow-lg whitespace-nowrap min-w-max w-fit">
              <button
                onClick={() => setActiveTab('available')}
                className={`px-4 py-2 rounded-xl font-body text-xs font-bold transition-all flex items-center justify-center gap-2 ${
                  activeTab === 'available'
                    ? 'bg-primary text-dark shadow-[0_8px_16px_rgba(142,255,113,0.25)] scale-[1.02]'
          ...
          {activeTab === 'available' && (
            <div className="flex flex-col md:flex-row gap-4 mb-8">
              <div className="relative flex-grow group">
                ...
                <input
                  className="w-full bg-dark-glass text-white border border-white/5 rounded-2xl py-4 pl-12 pr-4 font-body font-semibold focus:ring-2 focus:ring-primary/40 focus:border-primary/40 transition-all placeholder:text-gray-500 shadow-lg"
              ...
              <button
                ...
                className={`px-6 py-4 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg border ${
                  filterActive 
                    ? 'bg-primary/10 border-primary/40 text-primary shadow-[0_0_20px_rgba(142,255,113,0.15)]' 
                    : 'bg-dark-glass border-white/5 text-gray-400 hover:text-white hover:border-white/10'
                }`}
              >
          ...
          {activeTab === 'available' && (
            filteredQuizzes.length === 0 ? (
              <div className="text-center py-16 text-gray-400">No quizzes available.</div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                {mainQuiz && (() => {
                  ...
                  return (
                    <div className="lg:col-span-8 group relative">
                      <div className="absolute -inset-0.5 bg-gradient-to-r from-primary/20 to-secondary/20 rounded-[2rem] blur opacity-30 group-hover:opacity-60 transition duration-500"></div>
                      <div className="relative bg-dark-card rounded-[2rem] p-8 shadow-2xl border border-white/5 overflow-hidden">
                        ...
                        <div className="flex flex-col md:flex-row justify-between gap-6 mb-8">
                          <div className="space-y-4">
                            ...
                            <h3 className="text-4xl font-headline font-extrabold text-white leading-tight">{mainQuiz.title}</h3>
                            
                            <div className="flex flex-wrap gap-4">
                              <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 text-gray-300 text-sm font-medium border border-white/5">
                          ...
                          <div className="w-full md:w-auto flex flex-col justify-center shrink-0">
                            {availability.available ? (
                              <button
                                ...
                                className="w-full md:w-48 bg-primary hover:bg-primary/90 text-dark font-extrabold px-8 py-5 rounded-2xl shadow-[0_8px_24px_rgba(142,255,113,0.3)] hover:shadow-[0_12px_32px_rgba(142,255,113,0.4)] hover:-translate-y-1 transition-all flex items-center justify-center gap-2"
                              >
                                <span>Start Quiz</span>
                                <ArrowRight className="w-5 h-5" />
                              </button>
                            ) : (
                              <div className="w-full md:w-48 bg-white/5 border border-white/10 text-gray-400 font-bold px-6 py-5 rounded-2xl text-center flex items-center justify-center gap-2">
                        ...
                        {mainQuiz.attempts?.length > 0 && (
                          <div className="border-t border-white/10 pt-6 mt-4">
                            <h4 className="font-bold text-white mb-4 flex items-center gap-2"><RotateCcw className="w-4 h-4 text-primary"/> Previous attempts</h4>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {mainQuiz.attempts.slice(0, 2).map((att) => (
                                <div key={att.id} className="flex justify-between items-center p-4 bg-dark-glass rounded-xl border border-white/5 hover:border-primary/20 transition-colors">
                                  <div>
                                    <div className="flex items-end gap-2 mb-1">
                                      <span className="text-xl font-bold text-white">{att.percentage}%</span>
                                      <span className="text-xs text-gray-400 mb-1">({att.score}/{att.total_points})</span>
                                    </div>
                                  ...
                                  <button
                                    ...
                                    className="p-2.5 rounded-lg bg-primary/10 text-primary hover:bg-primary hover:text-dark transition-colors"
                                    title="View Result"
                                  >
                ...
                <div className="lg:col-span-4 space-y-4">
                  {sideQuizzes.map((quiz) => {
                    ...
                    return (
                      <div key={quiz.id} className="bg-dark-card rounded-[1.5rem] p-5 border border-white/5 hover:border-primary/30 hover:-translate-y-1 transition-all group shadow-lg">
                        <h4 className="font-headline font-bold text-lg text-white mb-1 leading-snug">{quiz.title}</h4>
                        <p className="text-xs text-primary/80 font-medium mb-4">{quiz.course_name}</p>
                        <div className="flex justify-between items-end mt-3 border-t border-white/5 pt-4">
                          ...
                          {availability.available ? (
                            <button
                              ...
                              className="w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center group-hover:bg-primary group-hover:text-dark transition-colors"
                              title="Start Quiz"
                            >
                              <PlayCircle className="w-5 h-5" />
                            </button>
                          ) : (
                            <span className="text-gray-600 text-xs flex items-center gap-1" title={availability.reason}><Clock className="w-3.5 h-3.5"/> N/A</span>
                          )}
                        </div>
                      </div>
                    );
                  ...
                  <div className="bg-dark-card rounded-[1.5rem] p-6 border border-white/5 relative overflow-hidden shadow-lg mt-4">
                    <div className="absolute -right-6 -bottom-6 w-32 h-32 bg-primary/5 rounded-full blur-2xl"></div>
                    <h4 className="font-headline font-bold text-sm uppercase tracking-widest text-gray-400 mb-5">Your Stats</h4>
                    <div className="space-y-3">
                      <div className="flex justify-between items-center p-4 bg-white/[0.02] border border-white/5 rounded-2xl">
                        <div className="flex items-center gap-3"><CheckCircle2 className="w-4 h-4 text-gray-400" /> <span className="text-sm text-gray-300 font-medium">Completed</span></div>
          ...
          {activeTab === 'completed' && (
            filteredCompleted.length === 0 ? (
              <div className="text-center py-16 text-gray-400">No completed quizzes yet.</div>
            ) : (
              <div className="space-y-4">
                {filteredCompleted.map((item) => (
                  <div key={item.attempt_id} className="group flex flex-col sm:flex-row items-center gap-6 p-6 bg-dark-card rounded-[2rem] border border-white/5 hover:border-primary/30 hover:shadow-[0_12px_40px_rgba(142,255,113,0.1)] transition-all duration-300">
                    <div className="w-16 h-16 shrink-0 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center group-hover:scale-110 group-hover:bg-primary/10 group-hover:border-primary/20 group-hover:text-primary transition-all">
                      <CheckCircle2 className="w-8 h-8 text-gray-400 group-hover:text-primary" />
                    </div>
                    
                    <div className="flex-1 text-center sm:text-left w-full sm:w-auto">
                      <h4 className="font-headline font-bold text-xl text-white mb-1">{item.quiz_title}</h4>
                      <p className="text-sm text-primary/80 font-medium mb-2">{item.course_name}</p>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 w-full sm:w-auto mt-4 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-t-0 border-white/10">
                      <div className="text-center sm:text-right">
                        <p className={`text-3xl font-headline font-black tracking-tight ${item.percentage >= (item.passing_score || 50) ? 'text-[#8eff71]' : 'text-[#ff7351]'}`}>
                          {item.percentage}%
                        </p>
                        <p className="text-xs text-gray-400 font-medium mt-1 uppercase tracking-wider">Score: {item.score}/{item.total_points}</p>
                      </div>
                      <button
                        onClick={() => handleViewResult(item.quiz_id, item.attempt_id)}
                        className="w-full sm:w-12 h-12 shrink-0 rounded-xl bg-white/5 text-white flex items-center justify-center gap-2 hover:bg-primary hover:text-dark transition-colors shadow-inner"

...
const getNotificationStyle = (title, content) => {
  ...
  if (lowerTitle.includes('contest') || lowerContent.includes('contest')) {
    return { emoji: <Trophy className="w-6 h-6 text-primary" />, category: 'Event', borderColor: 'bg-primary', iconBg: 'bg-primary/20', textColor: 'text-primary' };
  }
  if (lowerTitle.includes('grade') || lowerContent.includes('grade')) {
    return { emoji: <TrendingUp className="w-6 h-6 text-secondary" />, category: 'Grades', borderColor: 'bg-secondary', iconBg: 'bg-secondary/20', textColor: 'text-secondary' };
  }
  if (lowerTitle.includes('security') || lowerContent.includes('login')) {
    return { emoji: <ShieldCheck className="w-6 h-6 text-tertiary" />, category: 'Security', borderColor: 'bg-tertiary', iconBg: 'bg-tertiary/20', textColor: 'text-tertiary' };
  }
  return { emoji: <Info className="w-6 h-6 text-primary" />, category: 'Info', borderColor: 'bg-primary', iconBg: 'bg-primary/20', textColor: 'text-primary' };
};
    ...
    while ((match = linkRegex.exec(text)) !== null) {
      ...
      parts.push(
        <a
          ...
          className="inline-flex items-center gap-2 bg-gradient-to-r from-green-500/30 to-green-500/15 text-green-300 font-semibold px-4 py-2 rounded-xl border border-green-500/50 backdrop-blur-lg hover:border-green-400/70 hover:from-green-500/40 hover:to-green-500/25 transition-all shadow-md hover:shadow-lg mx-1"
        >
    ...
    <div className="min-h-screen bg-dark text-white font-body">
      ...
      <div className="md:ml-64 pb-24 md:pb-8">
        <div className="max-w-5xl mx-auto px-6 py-8">
          {/* Welcome Section */}
          <div className="relative overflow-hidden shadow-2xl p-8 rounded-[2rem] bg-dark-card border border-white/10 mb-10 group hover:border-white/20 transition-all duration-500">
            ...
            <div className="relative z-10">
              ...
              <h1 className="font-headline font-extrabold text-3xl md:text-4xl text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400 tracking-tight mb-4">
                {student?.name}
              </h1>
              <div className="flex flex-wrap gap-4">
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-dark-glass border border-white/5 text-gray-300 text-sm font-semibold">
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-dark-glass border border-white/5 text-white text-sm font-semibold">
          ...
          {!notifLoading && notifications.length > 0 && (
            <div className="mb-10 space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="font-headline font-extrabold text-2xl tracking-tight flex items-center gap-3 text-white">
              ...
              <div className="space-y-4">
                {notifications.map((notification) => {
                  ...
                  return (
                    <div
                      key={notification.id}
                      className={`relative overflow-hidden group rounded-[1.5rem] p-6 transition-all duration-300 ${isUnread ? "bg-dark-card shadow-[0_12px_40px_rgba(142,255,113,0.15)] border border-primary/30 hover:-translate-y-1 hover:shadow-2xl" : "bg-dark-glass/50 border border-white/5 hover:bg-white/5 opacity-80 hover:opacity-100"}`}
                    >
                      {isUnread && (
                        <div className={`absolute top-0 left-0 w-1 h-full ${borderColor} rounded-l-xl shadow-[0_0_12px_rgba(142,255,113,0.3)]`} />
                      )}
                      ...
                      <div className="flex justify-between items-start gap-3">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <h3 className={`font-headline font-bold text-lg ${isUnread ? 'text-white' : 'text-gray-400'}`}>
                              {notification.title}
                            </h3>
                      ...
                      <div className="text-gray-400 text-sm leading-relaxed mt-3 whitespace-pre-wrap" dir="auto">
                      ...
                      {isUnread && (
                        <div className="mt-3 flex justify-end">
                          <button
                            ...
                            className="text-xs font-bold text-primary/70 hover:text-primary transition-colors uppercase tracking-wider px-3 py-1 rounded-full bg-primary/10 hover:bg-primary/20"
                          >
                            Mark as read
                          </button>
                        </div>
          ...
          {/* Grades Section - بدون علامة Semester */}
                  ...
                  return (
                    <div key={idx} className="relative overflow-hidden group bg-dark-card border border-white/5 rounded-[1.5rem] p-6 hover:border-primary/40 hover:-translate-y-1.5 hover:shadow-[0_12px_40px_rgba(142,255,113,0.15)] transition-all duration-500">
                      ...
                      <div className="flex justify-between items-start mb-5 relative">
                        <div>
                          <h4 className="font-headline font-bold text-lg leading-tight mb-1 text-white">{grade.course_name}</h4>

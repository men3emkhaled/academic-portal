...
import { Terminal, Lock, Fingerprint, ArrowRight, ShieldCheck, Activity, Cpu, Eye, EyeOff } from 'lucide-react';
            ...
            <div
               ...
               style={{
                  background: isDarkMode
                     ...
                     : 'radial-gradient(ellipse at top right, rgba(16,185,129,0.05), transparent 50%), radial-gradient(ellipse at bottom left, rgba(20,184,166,0.05), transparent 50%)',
               }}
            />
         </div>
            ...
            >
               ...
               >
                  ...
                  <div className="text-start mb-6 lg:mb-8">
                     ...
                     <p className="mt-2 text-xs sm:text-sm font-bold text-gray-400 uppercase tracking-widest">{t('auth.admin_portal')}</p>
                  </div>

                  <form onSubmit={handleSubmit} className="space-y-6 lg:space-y-8">
                     <div className="space-y-6">
                        <div className="space-y-3">
                           <label className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-400 ms-6">{t('auth.admin_id')}</label>
                           <div className={`group relative flex items-center rounded-[1.75rem] border transition-all duration-500 focus-within:ring-8 focus-within:ring-emerald-500/5 ${isDarkMode ? 'bg-black/40 border-white/5 focus-within:border-emerald-500' : 'bg-gray-50 border-gray-100 focus-within:border-emerald-500 shadow-inner'}`}>
                              <Terminal className={`ms-6 w-5 h-5 transition-colors ${isDarkMode ? 'text-white/10 group-focus-within:text-emerald-500' : 'text-gray-300 group-focus-within:text-emerald-500'}`} />
                              <input
                                 type="text"
                        ...
                        <div className="space-y-3">
                           ...
                           <div className={`group relative flex items-center rounded-[1.75rem] border transition-all duration-500 focus-within:ring-8 focus-within:ring-emerald-500/5 ${isDarkMode ? 'bg-black/40 border-white/5 focus-within:border-emerald-500' : 'bg-gray-50 border-gray-100 focus-within:border-emerald-500 shadow-inner'}`}>
                              <Fingerprint className={`ms-6 w-5 h-5 transition-colors ${isDarkMode ? 'text-white/10 group-focus-within:text-emerald-500' : 'text-gray-300 group-focus-within:text-emerald-500'}`} />
                              <input
